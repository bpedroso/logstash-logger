...
public class LogRunner {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Runnable taskWarn = () -> {
			int i = 0;
			while(true) {
				LOGGER.warn("JMSClient Sent item blah " + i++ + " -> WARN");
				LOGGER.warn("Infinite loop!! Stop the proccess !!!");
				try {
					Thread.sleep(5000);
				} catch (Exception e) {
					LOGGER.error("Throws exception", e);
		...
		Runnable taskError = () -> {
			int i = 0;
			while(true) {
				LOGGER.error("Infinite loop!! Stop the proccess !!!");
				LOGGER.error("Shit on the process " + i++);
				LOGGER.error("Throws exception", new RuntimeException());
				try {
					Thread.sleep(10000);
				} catch (Exception e) {
					LOGGER.error("Throws exception", e);
